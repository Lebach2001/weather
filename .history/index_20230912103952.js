...
async function readApi(datas){
    ...
    button.addEventListener("click", function(){
        ...
        renderToChart(lat, lon, location);
    })
...
function renderWeatherNow(data, location){
    ...
    <div class="container-information">
        <div class="tempurature">
            ...
            <h1>${Math.round((data.current.weather[0].temp) -273.15)} &deg;C</h1>
        </div>
        <div class="description">
            <p>mây: ${data.current.clouds}%</p>
            <p>Tốc độ gió:${data.current.wind_speed}m/s</p>
        </div>
    </div>
    <div class="container-more">
        <p>Độ ẩm:  ${data.current.humidity}%</p>
        <p>Cảm thấy như:  ${Math.round((data.current.feels_like) - 273.15)} &deg;C</p>
        <p>Áp suất:  ${data.current.pressure} hPa</p>
    </div>`;
...
async function renderToChart(lat, lon, location){
    ...
    showChart(data);
    showDaily(data);
    renderWeatherNow(data, location)
}
...
if (localStorage.getItem("lc")) {
    ...
    renderToChart(21.0245, 105.8412, 'Ha Noi')
}
